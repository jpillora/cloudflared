...
1. **Tiny hand-maintained diff.** The only *hand-written* divergence from upstream is **2 files**,
   so re-syncing the logic stays cheap. The jpillora namespace rename spans ~182 files but is 100%
   mechanical — regenerated by `scripts/rename-fork.sh`, never hand-edited (see "Re-syncing").
...
### 3. `embed/examples/fileserver/main.go` (new)
...
git cherry-pick <embed-patch commit(s)>              # embed.go (new) + cmd.go (guarded hunks)
...
Only `cmd/cloudflared/tunnel/cmd.go` can conflict (during the cherry-pick), and only around the
guarded blocks (`sentry.Init`, `waitForSignal`, `notifySystemd`, the autoupdater goroutine, and the
`observer := connection.NewObserver(...)` line). `embed.go` is purely additive. If upstream renames
`StartServer`, `tunnelFlags`, `buildRunCommand`, `ParseToken`, `Init`, or the `connection.Observer`
sink API, update `embed.go` to match. Also re-copy the four dependency `replace` directives (see
"Consuming this fork") if upstream changes them.
