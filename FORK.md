...
1. **Tiny hand-maintained diff.** The only *hand-written* divergence from upstream is **5 files**
   (`cmd/cloudflared/tunnel/embed.go` + `ingress/memory_origin.go` new; `cmd/cloudflared/tunnel/cmd.go`
   + `connection/observer.go` + `ingress/origin_service.go` edited), so re-syncing the logic stays
...
git cherry-pick <embed-patch commit(s)>              # embed.go + ingress/memory_origin.go (new); cmd.go, observer.go, ingress/origin_service.go (edited)
...
`cmd/cloudflared/tunnel/cmd.go`, `connection/observer.go`, and `ingress/origin_service.go` are the
files that can conflict (during the cherry-pick): `cmd.go` around the guarded blocks (`sentry.Init`,
`waitForSignal`, `notifySystemd`, the autoupdater goroutine, the `observer := connection.NewObserver(...)`
line, and the `emb.onConfig` orchestrator hook), `observer.go` around `dispatchEvents` (the
sink-drain-before-event fix), and `origin_service.go` around `newHTTPTransport`'s `unixSocketPath`
case (the in-memory-origin dial). `embed.go` and `ingress/memory_origin.go` are purely additive. If upstream renames `StartServer`, `tunnelFlags`, `buildRunCommand`, `ParseToken`, `Init`,
