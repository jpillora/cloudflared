...
1. **Tiny hand-maintained diff.** The only *hand-written* divergence from upstream is the embed
   API and its hooks (`cmd/cloudflared/tunnel/embed.go` + `ingress/memory_origin.go` new;
   `cmd/cloudflared/tunnel/cmd.go` + `connection/observer.go` + `ingress/origin_service.go` +
   `orchestration/config.go` + `orchestration/orchestrator.go` edited, plus their tests and the
   `embed/examples/fileserver` example), so re-syncing the logic stays cheap. The jpillora namespace
   rename spans ~183 files but is 100% mechanical — regenerated by `scripts/rename-fork.sh`, never
...
before `NewOrchestratorWithHTTPRequestInterceptor`). Verified end-to-end against a dashboard-routed named tunnel: with
...
*regenerated* from `master` rather than merged (this keeps the ~183-file rename from ever producing
merge conflicts):
...
# Rebuild embed = master + the embed patch + the mechanical rename:
git checkout embed
git reset --hard master
git cherry-pick <embed-patch commit(s)>              # embed.go + ingress/memory_origin.go (new); cmd.go, observer.go, ingress/origin_service.go, orchestration/{config,orchestrator}.go (+ tests) edited
...
Notes from the 2026-09-19 re-sync (`master` `c06b2d43` → `be3ac127`, 62 upstream commits):
- Upstream removed `vendor/` (`TUN-10829`) — do **not** cherry-pick the old vendored-`bufconn`
  hunks; `ingress/memory_origin.go` imports `bufconn` from the `grpc` module instead and
  `go mod tidy` promotes it to a direct dependency. Keep upstream's `/vendor` `.gitignore` line.
- `TASKS.md` is fork-local and gitignored — if a cherry-picked commit re-adds it, finish with the
...
the `orchestratorConfig.IgnoreRemoteConfig` set, and the `NewOrchestratorWithHTTPRequestInterceptor`
call), `observer.go` around `dispatchEvents` (the
